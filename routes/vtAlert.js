...
app.get("/vt-alert", async (req, res) => {
  var filter = {};
  if (req.query.customerCode) {
    filter.customerCode = req.query.customerCode;
  }
  try {
    const checkEquipment = await vtEquipment.find(filter);
    ...
    return res.status(400).json({
      message: "Failed get data. something wrong!",
      error: error.tostring(),
    });
  }
});

app.get("/cron-vt-alert", async (req, res) => {
  try {
    const checkConnection = await axios.get(
      `https://api.volvotrucks.com/alert/vehiclealerts`,
      {
        headers: {
          Accept: "application/x.volvogroup.com.vehiclealerts.v1.1+json; UTF-8",
        },
        auth: {
